...
async function getPost(req, res, next) {
  ...
  try {
    ...
    // Increment views only if post is found
    post.views += 1;
    await post.save();

    res.post = post;
    return next();  // Ensure next() is called only once post is found

  } catch (error) {
    console.error("Error finding post:", error.message);
    return res.status(500).json({ message: "Error finding post: " + error.message });
  }
...
router.delete("/:postId/comment/delete/:commentId", auth, async (req, res) => {
  ...
  try {
    ...
    if (comment.author.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "ไม่สามรถลบได้ ผู้ใช้ไม่ถูกต้อง" })
    }
